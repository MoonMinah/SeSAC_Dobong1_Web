...
function encrypt(text) {
  //1. dkaghghk rorcp todtjd
  //const cipher = crypto.createCipheriv(알고리즘, 키, iv);
  const cipher = crypto.createCipheriv(algorithm, key, iv);
  //2. 실제 데이터 암호화 작업
  //let encrypted = cipher.update(암호화할데이터, 입력인코딩, 출려인코딩);
...
function decrypt(encryptedText) {
  ...
  // 2. 실제 데이터 복호화
  // base64로 인코딩된 문자열이 utf8형태로 복호화 된다.
  let decrypted = decipher.update(encryptedText, "base64", "utf8");
  // 3. final을 이용해 최종 결과 생성
...
console.log("복호화 됨", decryptedMessages);
